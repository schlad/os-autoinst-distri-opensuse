...
for my $line (@lines) {
    # Track the current group like in the parsing loop
    if ($line =~ /^\#\s*selftests:\s+cgroup:\s+(\S+)/) {
        $current_group_for_update = $1;
    ...
    if ($line =~ /^not ok\s+(\d+)\s+selftests:\s+cgroup:\s+(\S+)/) {
        ...
        if ($group_all_known{$grp}) {
            $self->{result} = 'softfail';
            record_info("Known issue", "All failed subtests in $grp are known issues; promoting top-level");
            $line = "ok $num selftests: cgroup: $grp # TODO Known issue";
        }
    }
