...
my $tap_file = '/tmp/nvme-cli_results.tap';
...
sub run {
    ...
    die 'No tests to run.' unless @tests;

    assert_script_run("rm -f $tap_file ${tap_file}.stderr");

    foreach my $test (@tests) {
        record_info('TEST', $test);
        # tap_runner.py always exits 0 and never prints a "file .." header of
        # its own, so prepend one here: OpenQA::Parser::Format::TAP requires
        # the first line of each embedded test file to match that pattern
        # before it will parse any of the following ok/not ok lines.
        assert_script_run("echo '$test.tap ..' >> $tap_file");
        script_run("python3 tests/tap_runner.py --start-dir tests $test >> $tap_file 2>> ${tap_file}.stderr", 300);
    }

    parse_extra_log(TAP => $tap_file);
    upload_logs("${tap_file}.stderr");

    my $failed = script_run("grep -q '^not ok' $tap_file");
    if (defined $failed && $failed == 0) {
        record_info('RESULT', 'One or more nvme-cli tests failed', result => 'fail');
        $self->result('fail');
    }
}
...
Example:
  ...
  nvme-cli:
      ...
      - product: sle:16\.1$
        skip: 1
        message: DSM not supported on this platform
